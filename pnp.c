        ...
        if (( IrpStack->Parameters.QueryId.IdType == BusQueryDeviceID ) 
         ...
         || ( IrpStack->Parameters.QueryId.IdType == BusQueryInstanceID )) {
            if (DeviceData->HardwareIDs) {
                ULONG length = 1024;  // No reason to be as long as this
                ASSERT( NT_SUCCESS( Game_CheckHardwareIDs (DeviceData->HardwareIDs,
                                    &length, FDO_FROM_PDO (DeviceData) ) ) );
            } else {
...
/*++
Routine Description:
    Check that the hardware ID we've been given is sane.
--*/
{
    PWCHAR                      pwszId;
    ULONG                       length;
    ...
    //
    // Loop through all hardware IDs until the double NULL terminator
    //
    while (*pwszId != UNICODE_NULL) {
...
#endif
        //
        // Hardware ID cannot start with a separator
        //
        if (*(pwszId++) == OBJ_NAME_PATH_SEPARATOR) {
            Game_KdPrint (FdoData, GAME_DBG_PNP_ERROR, 
                          ("hardware ID \"%ws\" invalid (missing bus ID)\n",pwszLastId));
        ...
        if (*(pwszId++) != OBJ_NAME_PATH_SEPARATOR) {
            Game_KdPrint (FdoData, GAME_DBG_PNP_ERROR, 
                          ("hardware ID \"%ws\" invalid (not in bus\\device format)\n",pwszLastId));
            return STATUS_INVALID_PARAMETER;
        }

        //
        // Device ID cannot start with a separator or NULL
        //
        if (*pwszId == OBJ_NAME_PATH_SEPARATOR) {
            Game_KdPrint (FdoData, GAME_DBG_PNP_ERROR, 
                          ("hardware ID \"%ws\" invalid (has duplicate separators)\n",pwszLastId));
            return STATUS_INVALID_PARAMETER;
        }
        if (*pwszId == UNICODE_NULL) {
            Game_KdPrint (FdoData, GAME_DBG_PNP_ERROR, 
                          ("hardware ID \"%ws\" invalid (missing device ID)\n",pwszLastId));
            return STATUS_INVALID_PARAMETER;
        }

        //
        // Find end of hardware ID
        //
        while (*(pwszId++) != UNICODE_NULL) {
            ;
        }
    ...
    length = (ULONG) (pwszId - pwszOrgId) + 1;

    //
    // Bad to have already touched memory past the limit but 
    // make sure we don't ignore this
    //
    if (length > *puLenLimit) {
        Game_KdPrint (FdoData, GAME_DBG_PNP_ERROR, 
                      ("hardware IDs invalid (length exceeds buffer size)\n"));
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (length < 3) {
        Game_KdPrint (FdoData, GAME_DBG_PNP_ERROR,
                      ("hardware ID is empty string\n"));
        return STATUS_INVALID_PARAMETER;
    }

    //
    // Copy the actual length back to the caller
    //
    *puLenLimit = length;
...
--*/
{
    ...
    //
    // A valid hardware ID multi sz (<bus>\\<device>\0\0) > 4 WCHARS
    //
    if (length < 5) {
        Game_KdPrint (FdoData, GAME_DBG_PNP_ERROR,
                      ("Expose with invalid hardware IDs\n"));
        return STATUS_INVALID_PARAMETER;
    }
...
--*/
{
    ...
    if (ExposeSibling->HardwareIDs) {
        ...
        // of REGSTR_VAL_MAX_HCID_LEN should be plenty.
        //
        length = 1024;
